...
namespace MeshCombineStudio
{
    ...
    public class MeshCombinerEditor : Editor
    {
        ...
        void DrawInspectorGUI()
        {
            ...
            // if (showMCSCavesIcon.boolValue)
            {
                if (meshCombiner.mcsCavesIcon == null || meshCombiner.deIcon == null)
                {
                    string path = (AssetDatabase.GetAssetPath(MonoScript.FromMonoBehaviour(meshCombiner)).Substring(0, 25) + "Sources/");
                    meshCombiner.mcsCavesIcon = AssetDatabase.LoadAssetAtPath(path + "MCSCavesIcon.jpg", typeof(Texture)) as Texture;
                    meshCombiner.deIcon = AssetDatabase.LoadAssetAtPath(path + "deIcon.jpg", typeof(Texture)) as Texture;
                    meshCombiner.tcIcon = AssetDatabase.LoadAssetAtPath(path + "tcIcon.jpg", typeof(Texture)) as Texture;
                    meshCombiner.wcIcon = AssetDatabase.LoadAssetAtPath(path + "wcIcon.jpg", typeof(Texture)) as Texture;
                ...
                {
                    if (GUI.Button(rect, new GUIContent(string.Empty, meshCombiner.mcsCavesIcon, "Click to go to 'MCS Caves & Overhangs' Extension.\n\nThis 'Mesh Combine Studio 2' extension gives the best performance possible by using the easiest way to get amazing looking Caves and Overhangs on any terrain solution. You can use any Rock Asset pack from the Unity Asset Store and stack them together to create caves and overhangs, and this extension will combine all rocks and remove all inside polygons that are never visible. Resulting in a ~60-80% polygon removal + combining which gives unbeatable performance and lightmap texture reduction. Our arenas in DRONE are optimized with this MCS extension. It can also be used on any other kind of closed meshes like a level consisting out of snapped cubes.")))
                    {
        ...
        bool DisplayVertsAndTrisInfo()
        {
            ...
            GUIDraw.PrefixAnd2Labels(new GUIContent(" "), new GUIContent("Original*", "This can show a lower number than `Original` count below, because if a cell has only 1 mesh it won't be combined and counted here, while `Original` below does count it."), labelWidth, 
                new GUIContent("Combined*", "This can show a lower number than `Combined` count below, because if a cell has only 1 mesh it won't be combined and counted here, while `Combine` below does count it as a new draw call."));
